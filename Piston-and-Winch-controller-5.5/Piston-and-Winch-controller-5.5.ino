...
/*
  28th April 2021. This version for piston and winch only.
...
void setup() {
  ...
  */
  ...
  //Piston and winch. This will be used for piston tank initial setup (Comment out 1 of the next delay lines for on /off winch control)
  //Use this delay for piston tank
  delay(35000); //35 second delay to get piston tank empty from the start.
  //Use this delay for winch
  //delay(1000);
...
void loop() {
  ...
  // Piston and winch. This section is added for winch use, stick down and stick up run motor in different directions, stick centre gives motor off
  // Substitute constant value for count to make switching with winch work. Comment out only the next line for piston tanks
  //count = 50;
  // Piston and winch. Comment out one of the next 2 lines
  //int band = 60; // Set the off-band for the winch
  int band = 15; // Set the off-band for the piston tank
  ...
  // If absolute value of errsig is less than twice the off-band , stop the motor
