...
void setup() {
  ...
  */
  ...
  //Set Hall switch counter to zero
...
void loop() {
  ...
  // Set constant value for Hall switch count in pump mode
  ...
  if ((errsig < 10) && (errsig > -10)) // Set from 15 to 10 in Aug 2020
  {
    digitalWrite(in1, HIGH);   //Swap in1 & in2 to reverse motor
    digitalWrite(in2, LOW);
  ...
  // To prevent failsafe from operating, swap the comment on the next 2 lines
  // failsafe_count = 1;
